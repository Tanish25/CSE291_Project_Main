...
void PlaceDB::add_def_component(DefParser::Component const& c) {
  ...
  if (!limbo::iequals(macro.className(), "CORE") && !limbo::iequals(macro.className(), "BLOCK")) {
    ...
    //node.setStatus(PlaceStatusEnum::FIXED);
        ...
        if (!(node.status() == PlaceStatusEnum::FIXED || node.status() == PlaceStatusEnum::PLACED)) {//Tanish
        std::cerr << "[WARNING] Changing status of non-CORE/BLOCK component '" << c.comp_name//Tanish
                  << "' with class '" << macro.className()//Tanish
                  << "' to FIXED (was " << node.status() << ")" << std::endl;//Tanish
        node.setStatus(PlaceStatusEnum::FIXED);//Tanish
  }
